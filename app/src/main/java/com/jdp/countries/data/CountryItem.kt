...
@Parcelize
class CountryItem : Parcelable, Comparable<CountryItem> {

    var id: Int? = null
    var name: Name? = null
    var independent: Boolean? = null
    var unMember: Boolean? = null
    var currencies: HashMap<String, CurrencyInfo> = HashMap()
    ...
    override fun compareTo(other: CountryItem): Int {
        return name?.official?.let {
            other.name?.official?.compareTo(it)
        } ?: kotlin.run { return 0 }
    }

    fun checkItemEquals(item: CountryItem): Boolean = item.name == name
    fun checkContentEquals(item: CountryItem): Boolean = item == this
}